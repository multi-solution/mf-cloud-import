...
このプラグインは**機能の多さより「事故を起こさないこと」を**優先しています。
...
- **Claude Code**（ターミナル / デスクトップアプリ / IDE拡張のいずれか）
  ...
  - → 詳しくは [データの扱い](#データの扱い)
- **Git**（プラグインを GitHub から取り込むために使います）
- **Python 3.9 以上を推奨**（3.8でも動作確認済み。追加ライブラリ不要・標準ライブラリのみ）
  - Git・Python の入れ方は、[導入マニュアル](docs/導入マニュアル.md)の**基本編 Step 1-2**に Mac／Windows 別で載せています
  - Mac は「コマンドライン・デベロッパ・ツール」を入れると、Git と Python が一度にそろいます
- **マネーフォワード クラウド会計**のアカウント
- 証憑の自動添付を使う場合：**マネーフォワードの開発者アプリ登録**（後述）

> **Windowsをお使いの場合**：Python は **Python Install Manager**（Microsoft Store または python.org）で入れると、
> 本書と同じ `python3` コマンドが使えます。従来型インストーラーで入れた場合は `python` または `py` に読み替えてください。
> なお、Windows での動作はまだ十分に検証できていません。
