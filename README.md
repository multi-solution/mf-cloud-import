...
記帳された仕訳は、マネーフォワードの仕訳帳に **「外部連携（API）」** と表示されます。
勘定科目・税区分・インボイス区分（軽減8%・80%控除）まで判定されます。
...
記帳した仕訳に、領収書そのものが添付されます。
ファイル名も「日付＿相手先＿内容＿金額」に整えて保存するので、あとから日付・取引先・金額のどれからでも探せます。
...
> 手順の詳細は **[導入マニュアル](docs/導入マニュアル.md#step-3プラグインを入れる3分)** にあります。
