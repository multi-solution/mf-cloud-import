...
マネーフォワード クラウド会計へ、**領収書を読み取って記帳し、証憑を仕訳に添付する**
Claude Code プラグインです。

銀行・クレジットカードの連携明細と手元の領収書を突き合わせ、勘定科目・税区分・
インボイス区分を判定して登録し、領収書PDFを仕訳に紐付けます。
ファイル名は「日付＿相手先＿内容＿金額」に整えて保存します。

> ⚠️ **本プラグインは、株式会社マネーフォワードの公式製品ではありません。**
> 株式会社 multi-solution が独自に開発した**非公式**のツールです。
> 「マネーフォワード クラウド会計」は株式会社マネーフォワードの登録商標です。
> **本プラグインについてのお問い合わせを、マネーフォワード社へ行わないでください。**
> ご質問・不具合の報告は、このリポジトリの Issues へお願いします。

> 📘 **はじめての方へ**：専門用語を使わない手順書があります → [導入マニュアル](docs/導入マニュアル.md)
> （基本編20分で記帳の自動化まで。難しい工程は応用編に分離してあります）
...
- **Claude Code**（ターミナル / デスクトップアプリ / IDE拡張のいずれか）
  ...
  - **顧問先など他者のデータを扱う場合は、Team プラン以上をおすすめします**（→ [データの扱い](#データの扱い)）
...
- **業務で他者のデータを扱うなら、Team プラン以上をおすすめします。**
- Pro / Max をお使いの場合は、[プライバシー設定](https://claude.ai/settings/data-privacy-controls) で
  モデル改善へのデータ利用を**必ずオフ**にしてください（オフなら保持30日、オンだと5年です）。
...
- 本プラグインは**記録の作成を補助するツール**であり、**税務相談・税務代理を行うものではありません。**
- 勘定科目・税区分・インボイス区分の判定は**参考情報**です。
...
- **電子帳簿保存法への適合を保証するものではありません。** 適合は、証憑の保存場所や
  ご運用の全体で判断されるものです。ファイル名の整形は、あとから探しやすくするための機能です。
- **本プラグインは株式会社マネーフォワードの公式製品ではなく、同社とは無関係に開発されたものです。**
  同社は本プラグインについて一切の責任を負いません。同社へのお問い合わせはご遠慮ください。
- 「マネーフォワード クラウド会計」は株式会社マネーフォワードの登録商標です。
